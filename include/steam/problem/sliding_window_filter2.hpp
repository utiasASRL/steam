...
class SlidingWindowFilter2 : public Problem {
  struct Variable {
 ...
 public:
  using Ptr = std::shared_ptr<SlidingWindowFilter2>;

  static Ptr MakeShared(unsigned int num_threads = 1);
  SlidingWindowFilter2(unsigned int num_threads = 1);
  ...
  void addMotionPriorCostTerm(const BaseCostTerm::ConstPtr& cost_term) override;
 ...
 private:
  ...
  std::vector<BaseCostTerm::ConstPtr> motion_prior_cost_terms_;
  ...
  bool initialized_P0 = false;
  Eigen::MatrixXd P_check_k;
  Eigen::VectorXd c_k;
};
