...
int main(int argc, char **argv) {
  ...
  for (const auto &lm : lm_ins)
    dot_lm_ins.emplace_back(lgmath::se3::point2fs(lm.head<3>(), lm(3)) *
                            w_is_ins);
  ...
  // Initialize problem
  steam::OptimizationProblem problem(1);

  // Create vector state variable
  const auto T_sv_var =
      steam::se3::SE3StateVar::MakeShared(lgmath::se3::Transformation(T_sv));
  T_sv_var->locked() = true;

  const auto w_iv_inv_var = steam::vspace::VSpaceStateVar<6>::MakeShared(
      Eigen::Matrix<double, 6, 1>::Zero());
  const auto w_is_ins_eval =
      steam::se3::compose_velocity(T_sv_var, w_iv_inv_var);
  ...
  const auto meas_noise_model =
      std::make_shared<steam::StaticNoiseModel<1>>(meas_cov);
  for (size_t i = 0; i < rv_measurements.size(); ++i) {
    const auto &lm = lm_ins.at(i);
    const auto &rv = rv_measurements.at(i);
    const auto rv_error = steam::p2p::RadialVelErrorEvaluator::MakeShared(
        w_is_ins_eval, lm.head<3>(), rv);
    const auto cost_term = std::make_shared<steam::WeightedLeastSqCostTerm<1>>(
        rv_error, meas_noise_model, loss_function);
  ...
  const auto prior_noise_model =
      std::make_shared<steam::StaticNoiseModel<6>>(prior_cov);
  const auto prior_cost_term =
      std::make_shared<steam::WeightedLeastSqCostTerm<6>>(
          w_iv_inv_var, prior_noise_model, loss_function);
