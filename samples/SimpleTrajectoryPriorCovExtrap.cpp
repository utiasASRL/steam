//////////////////////////////////////////////////////////////////////////////////////////////
/// \file TrajectoryPriorCovExtrap.cpp
/// \brief A sample usage of the STEAM Engine library for a trajectory prior problem with
...
int main(int argc, char **argv) {
  ...
  keys.push_back(states.back().pose->getKey());            
  keys.push_back(states.back().velocity->getKey());

  // Get covariance from solver
  steam::BlockMatrix block_covariance = solver.queryCovarianceBlock(keys);
  Eigen::Matrix<double,12,12> covariance;
  covariance.block<6,6>(0,0) = block_covariance.copyAt(0,0);
  covariance.block<6,6>(6,0) = block_covariance.copyAt(1,0);
  covariance.block<6,6>(0,6) = block_covariance.copyAt(0,1);
  covariance.block<6,6>(6,6) = block_covariance.copyAt(1,1);

  // std::cout << block_covariance.copyAt(0,0) << std::endl << std::endl;
  // std::cout << block_covariance.copyAt(0,1) << std::endl << std::endl;
  // std::cout << block_covariance.copyAt(1,1) << std::endl << std::endl;
  // std::cout << covariance << std::endl << std::endl;

  // Extrapolate
  steam::Time current_time = states.back().time;
  std::vector<Eigen::Matrix<double,12,12>> extrap_cov;
  unsigned int n = 6;
  for (unsigned int i = 1; i <= n; ++i) {
    extrap_cov.push_back(
        traj.extrapCovariance(covariance, current_time + steam::Time(i*delT)));
  }

  // Print variance on x
  std::cout << "\nExtrapolated variance in x: ";
  for (unsigned int i = 0; i < n; ++i) {
    std::cout << extrap_cov.at(i)(0,0) << ", ";
  }

  // Print variance on x
  std::cout << "\nExtrapolated variance in yaw: ";
  for (unsigned int i = 0; i < n; ++i) {
    std::cout << extrap_cov.at(i)(5,5) << ", ";
  }
  std::cout << std::endl;
