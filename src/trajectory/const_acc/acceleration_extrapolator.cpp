...
bool AccelerationExtrapolator::active() const { return knot_->acceleration()->active(); }
...
auto AccelerationExtrapolator::value() const -> OutType {
  const Eigen::Matrix<double,6,1> xi_k1 = Phi_.block<6, 6>(12, 12) * knot_->acceleration()->value();
