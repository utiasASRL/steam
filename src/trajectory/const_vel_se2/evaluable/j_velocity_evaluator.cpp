...
auto JVelocityEvaluator::value() const -> OutType {
  return lgmath::se2::vec2jac(xi_->value()) * velocity_->value();
}
...
auto JVelocityEvaluator::forward() const -> Node<OutType>::Ptr {
  ...
  //
  const auto value = lgmath::se2::vec2jac(child1->value()) * child2->value();
...
void JVelocityEvaluator::backward(const Eigen::MatrixXd& lhs,
  ...
  if (velocity_->active()) {
    velocity_->backward(lhs * lgmath::se2::vec2jac(child1->value()), child2,
                        jacs);
  }
}
