...
auto JinvVelocityEvaluator::forward() const -> Node<OutType>::Ptr {
  ...
  //
  const auto value = lgmath::se3::vec2jacinv(child1->value()) * child2->value();
...
void JinvVelocityEvaluator::backward(const Eigen::MatrixXd& lhs,
  ...
  if (xi_->active()) {
    xi_->backward((0.5) * lhs * lgmath::se3::curlyhat(child2->value()) *
                      lgmath::se3::vec2jacinv(child1->value()),
                  child1, jacs);
  }

  if (velocity_->active()) {
    velocity_->backward(lhs * lgmath::se3::vec2jacinv(child1->value()), child2,
                        jacs);
  }
}
